...
namespace VanPhap.View
{
    public partial class NguoiNhanCauSieu : Form
    {
...

            {
                ...
                else
                {
                    ...
                    if (kiemtra.Equals(""))
                    {
                        ...
                        using (OleDbConnection connection = new OleDbConnection(strCon))
                        {
                            try
                            {
                                ...
                                connection.Open();
                                // Tạo câu lệnh INSERT
                                string query2 = "INSERT INTO tblChiTietSo ( ID, IDSo, NamNu, NamSinh, NamMat, AmLich , Sao, Han , HoTenUni,  PhapDanhUni)  VALUES (?,?,?,?,?,?,?,?,?,?)";
                                ...
                                double nammat = double.Parse(txt_NamMat.Text);
                    ...
                    else
                    {
                        ...
                        string phapdanh = txt_nickname.Text;
                        // Tạo đối tượng Command và liên kết với Connection
                        string query4 = "INSERT INTO tblChiTietSo (ID,IDSo, NamNu,NamSinh,NamMat, AmLich , Sao, Han , HoTenUni,  PhapDanhUni)   VALUES (?,?,?,?,?,?,?,?,?,?) ";
                        using (OleDbConnection connection = new OleDbConnection(strCon))
                        {
        ...
        private void NguoiNhanCauSieu_Load(object sender, EventArgs e)
        {
            txt_id_so.Text = DataFromForm1;
        }
